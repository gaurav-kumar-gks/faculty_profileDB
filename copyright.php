...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  ...
  try {
    ...
    $stmt = "select * from faculty_profile_research where rtype='cpr' and sno>$SNO and roll='$roll' order by sno asc limit 1";
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
          ...
          <div class="row">
            <div class="col-md-12" style="width: 65rem;">
              ...
              <div class="card">
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    // echo $roll;
                    $stmt = "SELECT * FROM faculty_profile_research WHERE roll='$roll' AND rtype='cpr' ORDER BY sno DESC;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (strlen(Input::get('CnameS')) > 0) {
                      ...
                      // echo $roll;
                      $stmt = "SELECT fname,dept,title, other, projectStatus, ref, juri, pdate FROM faculty_profile_research WHERE rtype = 'cpr' AND title LIKE '%$cname%' ORDER BY pdate DESC";
                      // echo $stmt;
                      $result = mysqli_query($conn, $stmt);
                      while ($row = mysqli_fetch_assoc($result)) {
                    ?>
              ...
              <div class="card">
                <div class="card-header">
                  <form action="copyright.php">
                    <h4><i class="fa fa-search mr-3"></i>Search by Jurisdiction</h4>
                    <div class="input-group">
                      <input type="text" name="CjuriS" required class="form-control" placeholder="Search By Jurisdiction">
                      <input type="submit" name="CjuriSearch" class="btn btn-secondary">
                    </div>
                  </form>
                </div>
              </div>
              ...
              <div class="table-responsive">
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    <?php
                    if (strlen(Input::get('CjuriS')) > 0) {
                      $roll = $user->data()->{'Roll No'};
                      $cjuri = Input::get('CjuriS');
                      $conn = mysqli_connect("localhost", "root", "jrtalent", "faculty_profile_db");
                      if (!$conn)
                        die("Unable to connect to database");

                      // echo $roll;
                      $stmt = "SELECT fname,dept,title, other, projectStatus, ref, juri, pdate FROM faculty_profile_research WHERE rtype = 'cpr' AND juri LIKE '%$cjuri%' ORDER BY pdate DESC";
                      // echo $stmt;
                      $result = mysqli_query($conn, $stmt);
                      while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (strlen(Input::get('CstatusS')) > 0) {
                      ...
                      // echo $roll;
                      $stmt = "SELECT fname,dept,title, other, projectStatus, ref, juri, pdate FROM faculty_profile_research WHERE rtype = 'cpr' AND projectStatus LIKE '%$cstatus%' ORDER BY pdate DESC";
                      // echo $stmt;
                      $result = mysqli_query($conn, $stmt);
                      while ($row = mysqli_fetch_assoc($result)) {
                    ?>
