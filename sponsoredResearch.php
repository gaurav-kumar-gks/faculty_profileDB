...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // delete 
    $stmt = "DELETE FROM `faculty_profile_research` WHERE roll='$roll' AND rtype='orp' AND title=$title AND other=$other AND rpi =$rpi AND rcopi = $rcopi AND funds=$funds AND pdate = $cyear;";
    //echo $stmt;
    // run delete query
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  try {
    ...
    // update query
    $stmt = "UPDATE `faculty_profile_research` SET title='$title', other='$other', rpi='$rpi', rcopi='$rcopi', funds=$funds, pdate='$cyear' WHERE roll='$roll' AND rtype='orp' AND  title=$titlePrev AND other=$otherPrev AND rpi=$rpiPrev  AND rcopi=$rcopiPrev AND funds=$fundsPrev AND pdate=$cyearPrev;";
    //echo $stmt;
    // run query
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
