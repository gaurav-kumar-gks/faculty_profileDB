...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // delete query
    $stmt = "DELETE FROM `faculty_profile_publications` WHERE roll='$roll' AND ptype='opub' AND title=$ctitle AND authors=$cauthors AND publisher=$cpublisher AND pdate=$cyear AND bookType=$cbook;";
    //echo $stmt;
    // run delete query
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
  ...
  // if ($validationC->passed()) {
    try {
     ...
     // $jins = DB::getInstance();
      ...
      $stmt="select * from faculty_profile_publications where sno>$SNO and roll='$roll' and ptype='opub' order by sno asc limit 1";
      $result=mysqli_query($conn,$stmt);
      $count=0;
      $val=0;
...
if (Input::exists() && isset($_POST['edit'])) {
  try {
    ...
    // update query
    $stmt = "UPDATE `faculty_profile_publications` SET title='$ctitle', authors='$cauthors', publisher='$cpublisher', pdate='$cyear', onlineLink='$clink', bookType='$cbook' WHERE roll='$roll' AND ptype='opub' AND  title=$ctitlePrev AND authors=$cauthorsPrev AND publisher=$cpublisherPrev AND pdate=$cyearPrev AND onlineLink=$clinkPrev AND bookType=$cbookPrev;";
    //echo $stmt;
    // run query
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
          ...
          <div class="row">
            <div class="col-md-12" style="width: 65rem;">
              ...
              <div class="card">
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    // echo $roll;
                    $stmt = "SELECT * FROM faculty_profile_publications WHERE roll='$roll' AND ptype='opub' ORDER BY sno DESC;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
              ...
              <div class="card">
                ...
                <!-- Search 1- by TEXT BOOK name -->
                <form action="otherPublications.php">
                  <div class="input-group">
                    <input type="text" required name="CnameS" class="form-control" placeholder="Search By Text Book Title">
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('CnameS')) > 0) {
                    ...
                    $cnames->query("SELECT fname,dept,title,authors, publisher,CONCAT_WS('-', MONTH(pdate), YEAR(pdate)) AS day, bookType FROM faculty_profile_publications WHERE ptype = 'opub' AND title LIKE '%$cname%' ORDER BY pdate DESC");
                    ...
                    if ($cnames->count()) {
                      ...
                      foreach ($cnames->results() as $row) {
                        echo "<tr><td>$row->fname</td><td>$row->dept</td><td>$row->title</td><td>$row->authors</td><td>$row->publisher</td><td>$row->day</td><td>$row->bookType</td> </tr>\n";
                      }
                      echo "</tbody></table>";
                    }
                  }
                ...
                <!-- Search 1- by TEXT BOOK name ends -->
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('CpnameS')) > 0) {
                    ...
                    $cpnames->query("SELECT fname,dept,title,authors, publisher,CONCAT_WS('-', MONTH(pdate), YEAR(pdate)) AS day, bookType FROM faculty_profile_publications WHERE ptype = 'opub' AND publisher LIKE '%$cpname%' ORDER BY pdate DESC");
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('cyearS') && is_numeric(Input::get('cyearS'))) > 0) {
                    ...
                    $cyears->query("SELECT fname,dept,title,authors, publisher, CONCAT_WS('-', MONTH(pdate), YEAR(pdate)) AS day, bookType  FROM faculty_profile_publications WHERE ptype = 'opub' AND DATEDIFF(CURRENT_DATE, pdate)/365 < $cyear ORDER BY pdate DESC");
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('CfnameS')) > 0) {
                    ...
                    $cfnames->query("SELECT fname,dept,title,authors,  publisher,CONCAT_WS('-', MONTH(pdate), YEAR(pdate)) AS day, bookType FROM faculty_profile_publications WHERE ptype = 'opub' AND fname LIKE '%$cfname%' ORDER BY pdate DESC");
