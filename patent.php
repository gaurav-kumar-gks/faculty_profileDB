...
if (Input::exists() && isset($_POST['csubmit'])) {
  try {
    ...
    // insert query
    $stmt = "INSERT INTO `faculty_profile_research` (`fname`, `roll`, `dept`, `prog`, `email`, `aemail`, `rtype`, `title`, `other`, `rpi`, `rcopi`, `rlevel`, `remarks`, `funds`, `projectStatus`, `juri`, `ref`) VALUES ('$fname', '$roll', '$dept', '$prog', '$email', '$aemail', 'pat', '$title', NULL, NULL, NULL, NULL, NULL, NULL, '$projectStatus', NULL, '$refn');";
    // echo $stmt;
    // run insert query
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // delete query
    $stmt = "DELETE FROM `faculty_profile_research` WHERE roll='$roll' AND rtype='pat' AND title=$title  AND projectStatus=$projectStatus AND ref=$refn;";
    //echo $stmt;
    // run delete query
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  try {
    ...
    // update query
    $stmt = "UPDATE `faculty_profile_research` SET title='$title', projectStatus='$projectStatus', ref='$refn' WHERE roll='$roll' AND rtype='pat' AND  title=$titlePrev AND projectStatus=$projectStatusPrev AND ref=$refnPrev;";
              ...
              <div class="card">
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('BnameS')) > 0) {
                    ...
                    $bnames->query("SELECT fname,dept,title,ref, projectStatus FROM faculty_profile_research WHERE rtype = 'pat' AND title LIKE '%$bname%' ORDER BY lastUpdated DESC");

                    if ($bnames->count()) {
                      echo "<table class=\"table table-striped table-hover\">";
                      echo "<thead class=\"thead-inverse\">";
                      echo "<tr><th>Name</th><th>Dept</th><th>Title</th><th>Ref. No.</th><th>Status</th></tr></thead>";
                      echo "<tbody>";

                      foreach ($bnames->results() as $row) {
                        echo "<tr><td>$row->fname</td><td>$row->dept</td><td>$row->title</td><td>$row->ref</td><td>$row->projectStatus</td> </tr>\n";
                      }
                      echo "</tbody></table>";
                    }
                  }
                ...
                <div class="table-responsive">
                  ...
                  if (strlen(Input::get('CfnameS')) > 0) {
                    ...
                    $cfnames->query("SELECT fname,dept,title,ref, projectStatus FROM faculty_profile_research WHERE rtype = 'pat' AND fname LIKE '%$cfname%' ORDER BY lastUpdated DESC");
